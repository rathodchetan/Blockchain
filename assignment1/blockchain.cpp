...
void simulate(){
    ...
    while(!event_queue.empty()){
        ...
        if(tm.event_t == create_transaction and !break_sim){ // geting a event for creating a transaction
            ...
            int sender = tm.from;
            if (tm.mined_block->chain_length == 21) break_sim = true;
...
int main(int argc, char** argv){
    ...
    gv = new general_global_variables(10, 10, 600); // arg1- No of nodes in network, arg2 - mean inter-transaction time,
    ...
    gv->high_cpu_nodes = gv->node_count - gv->low_cpu_nodes;
    gv->mean_low_cpu_hashing_time = gv->mean_block_interarrival_time*(gv->low_cpu_nodes + 10*gv->high_cpu_nodes);
    ...
    for(int i=1; i<=gv->node_count; i++){
        ...
        nodes[i-1] = new node(i ,network_speed(features[0][i-1]) ,cpu_power(features[1][i-1]), gv->mean_transaction_time);
    } 
